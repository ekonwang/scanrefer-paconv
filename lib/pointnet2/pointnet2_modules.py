...
class PointnetSAModuleVotes(nn.Module):
    ...
    def __init__(
        ...
        if npoint is not None:
            ...
            raise ValueError('npoint should not be none.')
    ...
    def forward(self, xyz: torch.Tensor,
                ...
                lang_feat: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        r"""
        Parameters
        ----------
        xyz : torch.Tensor
        ...
        if not self.ret_unique_cnt:
            ...
            )  # (B, C, npoint, nsample), (B, 3, npoint, nsample), (B,npoint), (B, npoint, nsample)
        else:
            grouped_features, grouped_xyz, unique_cnt, _ = self.grouper(
                xyz, new_xyz, features
...
class PointnetSAModulePAConv(nn.Module):
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def __init__(
        ...
        ######### new #########
        self.use_lang_paconv = args.use_lang_paconv if args != None else False
        ######### ... #########
        ...
        ######### new #########
        self.mlp_module = paconv.SharedPAConv(mlps=mlp_spec, bn=bn, args=args)
        ######### ... #########
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        lang_feat : torch.Tensor
            (B, lang_hidden * (1 + int(self.use_bidir)))
        Returns
        -------
        new_xyz : torch.Tensor
            (B, npoint, 3) tensor of the new features' xyz
        ...
        for i in range(len(self.mlp_module)):
            if not self.ret_unique_cnt:
                _, grouped_xyz, inds = self.grouper(
                    xyz, new_xyz, new_features
                )  # (B, C, npoint, nsample), (B, 3, npoint, nsample), (B, npoint, nsample)
            else:
                _, grouped_xyz, unique_cnt, inds = self.grouper(
                    xyz, new_xyz, new_features
                )  # (B, C, npoint, nsample), (B,3,npoint,nsample), (B,npoint), (B, npoint, nsample)
            ...
            expanded_lang_feat = lang_feat.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, N1, K) if self.use_lang_paconv else None
            comb_features = torch.cat((new_features, xyz.transpose(1, 2)), dim = 1) if self.use_xyz and i == 0 else new_features
            new_features = self.mlp_module[i]((
                comb_features,
                grouped_xyz, 
                inds, 
                expanded_lang_feat 
            ...
            if self.pooling == 'max':
                ...
                rbf = torch.exp(-1 * grouped_xyz.pow(2).sum(1,keepdim=False) / (self.sigma**2) / 2) # (B, npoint, nsample)
                new_features = torch.sum(new_features * rbf.unsqueeze(1), -1, keepdim=True) / float(self.nsample) # (B, mlp[-1], npoint, 1)
            new_features = new_features.squeeze(-1)  # (B, out[i], npoint)
            xyz = new_xyz

        if not self.ret_unique_cnt:
            return new_xyz, new_features, fps_inds
        else:
            return new_xyz, new_features, fps_inds, unique_cnt
