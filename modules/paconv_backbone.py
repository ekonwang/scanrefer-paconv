...
class Pointnet2Backbone(nn.Module):
    ...
    def __init__(self, input_feature_dim=0, 
                paconv_backbone = False,
                # paconv_votenet = False, # ...
                # paconv_proposal = False, # ...
                ):
        super().__init__()
    ...
    def forward(self, data_dict):
        ...
        if self.paconv_backbone and "lang_emb" not in data_dict.keys():
            raise ValueError('use paconv in backbone without language embeddings.')

        # --------- 4 SET ABSTRACTION LAYERS ---------
        xyz, features, fps_inds = self.sa1(xyz, features, data_dict)
        data_dict['sa1_inds'] = fps_inds
        data_dict['sa1_xyz'] = xyz
        data_dict['sa1_features'] = features

        xyz, features, fps_inds = self.sa2(xyz, features, data_dict) # this fps_inds is just 0,1,...,1023
        data_dict['sa2_inds'] = fps_inds
        data_dict['sa2_xyz'] = xyz
        data_dict['sa2_features'] = features

        xyz, features, fps_inds = self.sa3(xyz, features, data_dict) # this fps_inds is just 0,1,...,511
        data_dict['sa3_xyz'] = xyz
        data_dict['sa3_features'] = features

        xyz, features, fps_inds = self.sa4(xyz, features, data_dict) # this fps_inds is just 0,1,...,255
